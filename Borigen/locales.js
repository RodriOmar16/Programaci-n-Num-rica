  ...
  actions: {
    ...
    */
    ...
    async getLocalesPadresHijos({}, datos){
      return await new Promise(async (resolve, reject) => {
        try{
          let localesPeticion = await fetch(`${config.BASE_URL}/locales/getLocalesPadresHijos?empresa_codigo=${datos.empresa_codigo}&sucursal_codigo=${datos.sucursal_codigo}&local_codigo_origen=${datos.local_codigo_origen}`, {
            method: 'GET',
            headers: {
              'Content-Type': 'application/json',
              'Authorization': store.state.long_token
